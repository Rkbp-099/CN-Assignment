...
int main()
{
    ...
    error_check = recvfrom(server_socket,dat,sizeof(dat),0, (struct sockaddr *) &client_address, &client_len);
    if(error_check == -1)
    {
    ...
    printf("Client sent character: %f\n", dat[0]);
    dat[0]+=1.5;
    ...
    error_check = sendto(server_socket, dat, sizeof(dat), 0, (struct sockaddr *) &client_address, client_len);
    if(error_check == -1)
    {
